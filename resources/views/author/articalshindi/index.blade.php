        ...
        <div class="content-wrapper">
            <div class="breadcrumbs-top d-flex align-items-center bg-light mb-3">
                ...
                <div class="add-back ml-auto mr-2">
                    <a href="javascript:void(0)" class="add-artical-hindi btn text-dark"><i class="fa fa-plus"></i> Add
                    </a>
                            ...
                            @foreach ($articals as $artical)
                                <tr>
                                    ...
                                    <td class="text-truncate">
                                        ...
                                        <a href="javascript:void(0)" class="edit-artical-hindi text-dark mr-2"
...
<script>
    function deleteConfirmation(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `{{ URL::to('author/manage-artical-hindi/${id}') }}`,
                    type: "DELETE",
                    dataType: "json",
    ...
    $(document).ready(function() {
        ...
        $(document).on("click", ".add-artical-hindi", function(event) {
            $.ajax({
                url: "{{ URL::to('author/manage-artical-hindi/create') }}",
                type: "GET",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", "#add-artical-hindi-btn", function(event) {
           ...
           // }
            ...
            formData.append('image', (typeof $('#image')[0].files[0] == 'undefined') ? '' : $('#image')[
                0].files[0]);
            $.ajax({
                url: "{{ URL::to('author/manage-artical-hindi') }}",
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
        ...
        $(document).on("click", ".edit-artical-hindi", function(event) {
            let id = $(this).attr('artical_id');

            $.ajax({
                url: `{{ URL::to('author/manage-artical-hindi/${id}/edit') }}`,
                type: "get",
                dataType: "json",
                success: function(result) {
                    if (result.success) {
        ...
        $(document).on("click", "#update-artical-hindi-btn", function(event) {
            //for (instance in CKEDITOR.instances) {
                //CKEDITOR.instances[instance].updateElement();
            //}
            $(this).attr('disabled', true);
            ...
            $.ajax({
                url: `{{ URL::to('author/manage-artical-hindi/${artical_id}') }}`,
                type: 'POST',
                processData: false,
                contentType: false,
                dataType: 'json',
